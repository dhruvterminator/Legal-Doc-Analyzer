...
const FileUpload: React.FC = () => {
  ...
  const fetchQuestions = async () => {
    try {
      ...
      setQuestions(data);
      if (data.length > 0) {
        setSelectedQuestion(data[0]); // default selection
      }
    } catch (error) {
      console.log('Error fetching questions:', error);
    }
  ...
  const handleExplanationClick = async () => {
    ...
    try {
      const encoded = encodeURIComponent(selectedResponse);
      const res = await fetch(
        `http://127.0.0.1:5000/contracts/paraphrase/${encoded}`
      );
      const data = await res.json();

      const htmlContent = data.map((p: string) => `<p>${p}</p>`).join('');
      document.getElementById('explanation')!.innerHTML = htmlContent;
    } catch (error) {
      console.log(error);
    }
      ...
      <div className="dashboard">
        <form onSubmit={handleFormSubmit} encType="multipart/form-data">
          <label className="drop-container">
            <span className="drop-title">Drop files here</span>
            or
            <input type="file" name="file" className="file-upload" required />
          </label>
